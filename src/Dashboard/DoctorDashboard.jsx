...
const PATIENTS = [
  { id: "P087b7513", name: "Matthew Schiavo",  age: 29, room: "B-101", diagnosis: "Hypertension",           condition: "Stable",   admitted: "20 Mar 2026" },
  { id: "Pa2567288", name: "Karim Belhaj",      age: 58, room: "B-102", diagnosis: "Acute Heart Failure",    condition: "Critical", admitted: "28 Mar 2026" },
  { id: "Pb1234567", name: "Lena Farhat",       age: 45, room: "B-103", diagnosis: "Arrhythmia",             condition: "Stable",   admitted: "22 Mar 2026" },
  { id: "Pc9876543", name: "Omar Slimane",      age: 67, room: "B-104", diagnosis: "Coronary Artery Disease",condition: "Caution",  admitted: "25 Mar 2026" },
  { id: "Pd1122334", name: "Nour Trabelsi",     age: 29, room: "B-105", diagnosis: "Mitral Valve Prolapse",  condition: "Stable",   admitted: "27 Mar 2026" },
];
...
const ALERTS_INIT = [
  { id: 1, patientName: "Karim Belhaj",  room: "B-102", type: "critical", message: "Heart rate 102 bpm — exceeds threshold",          time: "5m ago"  },
  { id: 2, patientName: "Karim Belhaj",  room: "B-102", type: "critical", message: "SpO₂ at 93% — possible respiratory compromise",    time: "8m ago"  },
  { id: 3, patientName: "Omar Slimane",  room: "B-104", type: "caution",  message: "BP 135/85 — elevated, review medication",          time: "22m ago" },
];

const APPOINTMENTS = [
  { id: 1, patientName: "Matthew Schiavo", room: "B-101", time: "09:30 AM", type: "Follow-up consultation",  status: "done"    },
  { id: 2, patientName: "Karim Belhaj",    room: "B-102", time: "11:00 AM", type: "Emergency review",        status: "done"    },
  { id: 3, patientName: "Lena Farhat",     room: "B-103", time: "01:30 PM", type: "ECG result review",       status: "pending" },
  { id: 4, patientName: "Omar Slimane",    room: "B-104", time: "03:00 PM", type: "Medication adjustment",   status: "pending" },
  { id: 5, patientName: "Nour Trabelsi",   room: "B-105", time: "04:30 PM", type: "Discharge assessment",    status: "pending" },
];

const LAB_RESULTS = [
  { id: 1, patientName: "Karim Belhaj",   test: "BNP Level",           result: "820 pg/mL",  ref: "<100 pg/mL",  flag: "high"   },
  { id: 2, patientName: "Matthew Schiavo",test: "HbA1c",               result: "5.8%",        ref: "<5.7%",       flag: "high"   },
  { id: 3, patientName: "Omar Slimane",   test: "LDL Cholesterol",     result: "142 mg/dL",  ref: "<100 mg/dL",  flag: "high"   },
  { id: 4, patientName: "Lena Farhat",    test: "TSH",                 result: "2.1 mIU/L",  ref: "0.4–4.0",     flag: "normal" },
  { id: 5, patientName: "Nour Trabelsi",  test: "CBC — Haemoglobin",   result: "13.2 g/dL",  ref: "12.0–16.0",   flag: "normal" },
];

const NOTES_INIT = [
  { id: 1, patientName: "Karim Belhaj",   date: "Today 10:45 AM",      note: "Patient presenting with acute decompensated heart failure. BNP markedly elevated. Initiated IV Furosemide. Monitoring fluid balance closely. Echo ordered." },
  { id: 2, patientName: "Matthew Schiavo",date: "Today 09:35 AM",      note: "BP well controlled on current regimen. HbA1c mildly elevated — referred for dietary counselling. Continue Metoprolol 50mg." },
  { id: 3, patientName: "Omar Slimane",   date: "Yesterday 03:00 PM",  note: "LDL elevated despite statin therapy. Increased Atorvastatin to 40mg. Lifestyle modification discussed. Follow-up in 4 weeks." },
];

const PROC_INIT = [
  { id: 1, patientName: "Karim Belhaj",   procedure: "Echocardiogram",       date: "30 Mar 2026", status: "scheduled"       },
  { id: 2, patientName: "Lena Farhat",    procedure: "24h Holter Monitor",   date: "1 Apr 2026",  status: "scheduled"       },
  { id: 3, patientName: "Matthew Schiavo",procedure: "Stress ECG",           date: "3 Apr 2026",  status: "scheduled"       },
  { id: 4, patientName: "Omar Slimane",   procedure: "Coronary Angiography", date: "5 Apr 2026",  status: "pending approval"},
];
...
function DoctorDashboard() {
 ...
 const addRx = async () => {
  ...
  const addProc = () => {
    ...
    setProc(p => [{ id: Date.now(), ...procForm, patientName: procForm.patient, status: "scheduled" }, ...p]);
    ...
    <div className="dd-root">
      ...
      <main className="dd-main">
        <header className="dd-topbar">
          <button className="dd-hamburger" onClick={() => setSidebar(s => !s)}>
            <span /><span /><span />
          </button>
                      ...
                      return (
                        <div key={p.id} className="dd-critical-card">
                          <div className="dd-critical-left">
                            <div className="dd-avatar dd-avatar--critical">{initials(p.name)}</div>
                            <div>
                          ...
                          {v && (
                            <div className="dd-critical-vitals">
                              {[["HR", v.hr, "bpm"], ["BP", v.bp, ""], ["SpO₂", v.spo2, "%"]].map(([l, val, u]) => (
                ...
                {APPOINTMENTS.map(a => (
                  <div key={a.id} className={`dd-appt-row ${a.status === "done" ? "done" : ""}`}>
                    ...
                    <div className="dd-appt-info">
                      ...
                      <div className="dd-appt-type">{a.type} · {a.room}</div>
                    </div>
                ...
                <>
                  ...
                  <div className="dd-patient-detail">
                    <div className="dd-patient-detail-header">
                      <div className="dd-avatar dd-avatar--accent dd-avatar--lg">{initials(selPatient.name)}</div>
                      <div className="dd-patient-detail-info">
                        <h2>{selPatient.name}</h2>
                        <p>{selPatient.room} · Age {selPatient.age} · Admitted {selPatient.admitted}</p>
                      ...
                      <>
                        ...
                        <div className="dd-vitals-row">
                          {[
                            ...
                            ["Blood Pressure",VITALS_MAP[selPatient.id].bp,   "mmHg"],
                            ["SpO₂",          VITALS_MAP[selPatient.id].spo2, "%"],
                            ["Temperature",   VITALS_MAP[selPatient.id].temp, "°C"],
                            ["Resp. Rate",    VITALS_MAP[selPatient.id].rr,   "br/min"],
                          ].map(([label, val, unit]) => (
                    ...
                    {/* Patient notes */}
                    {notes.filter(n => n.patientName === selPatient.name).length > 0 && (
                      <>
                        <SectionHeading>Clinical Notes</SectionHeading>
                        {notes.filter(n => n.patientName === selPatient.name).map(n => (
                    ...
                    {PATIENTS.map(p => (
                      <div key={p.id} className="dd-patient-card" onClick={() => setSelPat(p)}>
                        <div className="dd-patient-card-top">
                          <div className={`dd-avatar dd-avatar--${condColor(p.condition)}`}>{initials(p.name)}</div>
                        ...
                        <div className="dd-patient-card-admitted">Admitted {p.admitted}</div>
                      </div>
                    ))}
                  </div>
                </>
                    ...
                    {PATIENTS.map(p => {
                      ...
                      return (
                        <tr key={p.id} className={p.condition === "Critical" ? "dd-tr--critical" : ""}>
                          <td><div className="dd-table-patient"><div className={`dd-avatar dd-avatar--sm dd-avatar--${condColor(p.condition)}`}>{initials(p.name)}</div>{p.name}</div></td>
                          <td className="dd-mono">{p.room}</td>
                          <td className={`dd-mono ${v?.hr > 100 ? "dd-val--warn" : ""}`}>{v?.hr ?? "—"}</td>
                          <td className="dd-mono">{v?.bp ?? "—"}</td>
                          <td className={`dd-mono ${v?.spo2 < 95 ? "dd-val--critical" : ""}`}>{v?.spo2 ?? "—"}</td>
                          <td className={`dd-mono ${v?.temp > 38 ? "dd-val--warn" : ""}`}>{v?.temp ?? "—"}</td>
                          <td className={`dd-mono ${v?.rr > 20 ? "dd-val--warn" : ""}`}>{v?.rr ?? "—"}</td>
                ...
                {APPOINTMENTS.map(a => (
                  <div key={a.id} className={`dd-appt-full-card ${a.status === "done" ? "done" : ""}`}>
                    ...
                    <div className="dd-appt-full-body">
                      ...
                      <div className="dd-appt-full-meta">{a.type} · {a.room}</div>
                    </div>
            ...
            <div className="dd-section fade-in">
              <div className="dd-form-card">
                ...
                <div className="dd-form-grid">
                  <input className="dd-input" placeholder="Patient ID (e.g. P087b7513)"
                    value={rxForm.patient} onChange={e => setRxForm({...rxForm, patient: e.target.value})} />
                  <input className="dd-input" placeholder="Medication name"
                    value={rxForm.med} onChange={e => setRxForm({...rxForm, med: e.target.value})} />
                  <input className="dd-input" placeholder="Dosage (e.g. 50mg)"
                    value={rxForm.dose} onChange={e => setRxForm({...rxForm, dose: e.target.value})} />
                  <input className="dd-input" placeholder="Frequency (e.g. Once daily)"
                    value={rxForm.freq} onChange={e => setRxForm({...rxForm, freq: e.target.value})} />
                  <input className="dd-input" placeholder="Start date (YYYY-MM-DD)"
                    value={rxForm.startDate} onChange={e => setRxForm({...rxForm, startDate: e.target.value})} />
                  <input className="dd-input" placeholder="End date (YYYY-MM-DD)"
                    value={rxForm.endDate} onChange={e => setRxForm({...rxForm, endDate: e.target.value})} />
                </div>
                  ...
                  <div className="dd-rx-list">
                    {rx.map((m, i) => (
                      <div key={m.id || i} className="dd-rx-card">
                        ...
                        {m.startDate && <div className="dd-rx-dates">{m.startDate} → {m.endDate}</div>}
            ...
            <div className="dd-section fade-in">
              <div className="dd-form-card">
                ...
                <div className="dd-form-grid dd-form-grid--single">
                  <input className="dd-input" placeholder="Patient name"
                    value={noteForm.patient} onChange={e => setNoteForm({...noteForm, patient: e.target.value})} />
                  <textarea className="dd-input dd-textarea" placeholder="Write clinical note…" rows={4}
          ...
          {active === "procedures" && (
            <div className="dd-section fade-in">
              <div className="dd-form-card">
                ...
                <div className="dd-form-grid">
                  <input className="dd-input" placeholder="Patient name"
                    value={procForm.patient} onChange={e => setProcForm({...procForm, patient: e.target.value})} />
                  <input className="dd-input" placeholder="Procedure name"
                    value={procForm.procedure} onChange={e => setProcForm({...procForm, procedure: e.target.value})} />
                  <input className="dd-input" placeholder="Date (e.g. 1 Apr 2026)"
                    value={procForm.date} onChange={e => setProcForm({...procForm, date: e.target.value})} />
                </div>
              ...
              <div className="dd-proc-list">
                {procedures.map(p => (
                  <div key={p.id} className="dd-proc-card">
                    ...
                    <Badge
                      label={p.status}
                      variant={p.status === "scheduled" ? "normal" : p.status === "pending approval" ? "warn" : "default"}
                    />
                  </div>
                ))}
              </div>
