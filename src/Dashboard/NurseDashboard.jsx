...
const patients = [
  {
    ...
    room: "B-101",
    condition: "Stable",
    vitals: { hr: "78", bp: "118/76", spo2: "98", temp: "37.1" },
    alerts: 1,
  },
  {
    id: 2,
    ...
    room: "B-102",
    condition: "Critical",
    vitals: { hr: "102", bp: "145/90", spo2: "93", temp: "38.6" },
    alerts: 3,
  },
  {
    id: 3,
  ...
  },
  {
    ...
    room: "B-104",
    condition: "Caution",
    vitals: { hr: "88", bp: "135/85", spo2: "96", temp: "37.4" },
    alerts: 1,
  },
  {
    id: 5,
    ...
    room: "B-105",
    condition: "Stable",
    vitals: { hr: "68", bp: "110/70", spo2: "99", temp: "36.6" },
    alerts: 0,
  },
];
...
const alertsData = [
  ...
  },
  {
    ...
    room: "B-102",
    type: "critical",
    message: "SpO₂ dropped to 93% — check O₂ supply",
    time: "8m ago",
  },
  {
    id: 3,
    patient: "Omar Slimane",
    room: "B-104",
    type: "caution",
    message: "Blood pressure elevated — 135/85",
    time: "22m ago",
  },
  {
    id: 4,
    ...
    room: "B-102",
    type: "critical",
    message: "Temperature 38.6°C — possible fever",
    time: "30m ago",
  },
  {
    id: 5,
    ...
    room: "B-101",
    type: "info",
    message: "Medication due at 09:00 PM",
    time: "1h ago",
  },
];
...
const tasksInit = [
  {
    id: 1,
    text: "Check vitals — Room B-102 (Karim)",
    done: false,
    priority: "high",
  },
  {
    id: 2,
    text: "Administer Ramipril — Room B-104",
    done: false,
    priority: "high",
  },
  {
    id: 3,
    text: "Change wound dressing — Room B-103",
    done: false,
    priority: "medium",
  },
  {
    id: 4,
    text: "Submit shift handover report",
    done: false,
    priority: "medium",
  },
  ...
  {
    id: 6,
    text: "Update patient notes — Room B-101",
    done: true,
    priority: "low",
  },
];
...
const alertBorder = (t) =>
  t === "critical"
    ? "db-alert-card--critical"
    : t === "caution"
      ? "db-alert-card--caution"
      : "db-alert-card--info";
const alertDot = (t) =>
  t === "critical"
    ? "db-alert-dot--critical"
    : t === "caution"
      ? "db-alert-dot--caution"
      : "db-alert-dot--info";
const initials = (name) =>
  name
    .split(" ")
...
function NurseDashboard({ navigate }) {
  ...
  useEffect(() => {
    ...
    fetchAllAlerts();
    const iv = setInterval(fetchAllAlerts, 10000); // Update every 10s
    return () => clearInterval(iv);
  }, []);
  // Inside NurseDashboard.jsx
  useEffect(() => {
    const fetchAllMeds = async () => {
      const res = await authFetch(`${API}/medication/all`);
      if (res.ok) setMeds(await res.json());
    };
    if (active === "medications") fetchAllMeds();
  }, [active]);
  ...
  const toggleMed = (id) =>
    setMeds((p) => p.map((m) => (m.id === id ? { ...m, given: !m.given } : m)));
  const addTask = () => {
      ...
      <aside className="db-sidebar">
          ...
          {navItems.map((item) => (
            ...
            >
              ...
              {item.id === "alerts" && visAlerts.length > 0 && (
                <span className="db-nav-badge db-nav-badge--alert">
                  {visAlerts.length}
                </span>
              )}
              {item.id === "tasks" && (
                <span className="db-nav-badge db-nav-badge--task">
        ...
        <div className="db-sidebar-footer">
          ...
          <div>
            ...
            <div className="db-sidebar-sub">
              {nurseData?.id} · {nurseData?.role}
            </div>
          </div>
        </div>
      </aside>
      ...
      <main className="db-main">
        <header className="db-topbar">
          ...
          <div className="db-topbar-right">
            ...
            {navigate && (
              <button className="db-back-btn" onClick={() => navigate("home")}>
                ← Back
              </button>
            )}
          </div>
          ...
          <div className="db-section">
              ...
              <>
                <div className="db-stats-grid">
                  {[
                    ...
                    {
                      label: "Critical",
                      val: patients.filter((p) => p.condition === "Critical")
                        .length,
                    ...
                    {
                      label: "Active Alerts",
                      val: visAlerts.length,
                      sub: "Unresolved",
                      warn: visAlerts.length > 0,
                    },
                    {
                      label: "Tasks pending",
                    ...
                    <div key={i} className="db-stat-card">
                      ...
                      <div
                        className={`db-stat-value ${s.warn ? "db-stat-value--warn" : ""}`}
                      >
                        {s.val}
                      </div>
                      ...
                      .map((p) => (
                        <div key={p.id} className="db-critical-card">
                          <div className="db-critical-card-left">
                            <div
                              className="db-patient-avatar-sm"
                              style={{
                                background: "#FDEAEA",
                                color: "#E24B4A",
                              }}
                          ...
                          <div className="db-critical-vitals">
                            {[
                              ...
                              <div key={l} className="db-critical-vital-item">
                                <div className="db-critical-vital-label">
                                  {l}
                                </div>
                                <div className="db-critical-vital-val">
                                  {v}
                                  <span className="db-critical-vital-unit">
                                    {u}
                                  </span>
                                </div>
                              </div>
                          ...
                          <span className="db-badge db-badge--critical">
                            {p.condition}
                          </span>
                        </div>
                      ))}
                  ...
                  <div>
                    ...
                    {visAlerts.slice(0, 3).map((a) => (
                      <div key={a.id} className={`db-card db-card--${a.type}`}>
                        <div className="db-alert-patient">
                          {a.patient} ·{" "}
                          <span className="db-alert-room">{a.room}</span>
                        </div>
                  ...
                  <>
                    ...
                    >
                      <div className="db-patient-detail-header">
                        <div
                          className="db-patient-initials"
                          style={{ background: "#FDEAEA", color: "#E24B4A" }}
                        >
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div className="db-patient-meta">
                            {selPatient.room} · Age {selPatient.age}
                          </div>
                        </div>
                        <span
                          className={`db-badge ${condBadge(selPatient.condition)}`}
                        >
                          {selPatient.condition}
                        </span>
                      </div>
                      <div className="db-vitals-mini-grid db-vitals-mini-grid--4">
                        {[
                          ...
                          <div key={l} className="db-vitals-mini-cell">
                            ...
                            <div className="db-vitals-mini-val">
                              {v}
                              <span className="db-vitals-mini-unit">{u}</span>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    {alertsData
                      .filter((a) => a.patient === selPatient.name)
                      .map((a) => (
                        <div
                          key={a.id}
                          className={`db-card db-card--${a.type}`}
                        >
                          <div className="db-alert-msg">{a.message}</div>
                          <div className="db-alert-time">{a.time}</div>
                        </div>
                      ))}
                    {alertsData.filter((a) => a.patient === selPatient.name)
                      .length === 0 && (
                    ...
                    {patients.map((p) => (
                      ...
                      >
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div className="db-patient-card-sub">
                            {p.room} · Age {p.age}
                          </div>
                        </div>
                        <div className="db-patient-card-right">
                          <span
                            className={`db-badge ${condBadge(p.condition)}`}
                          >
                            {p.condition}
                          </span>
                          {p.alerts > 0 && (
                            <span className="db-patient-alert-txt">
            ...
            {active === "vitals" && (
              <>
                    ...
                    <tbody>
                      {patients.map((p) => (
                        <tr
                          key={p.id}
                          className={
                            p.condition === "Critical" ? "critical-row" : ""
                          }
                        >
                          ...
                          <td>
                            <span
                              className={`db-table-val ${parseInt(p.vitals.hr) > 100 ? "db-table-val--warn" : ""}`}
                            >
                              {p.vitals.hr}
                              <span className="db-table-unit">bpm</span>
                            </span>
                          </td>
                          <td>
                            <span className="db-table-val">{p.vitals.bp}</span>
                          </td>
                          <td>
                            <span
                              className={`db-table-val ${parseInt(p.vitals.spo2) < 95 ? "db-table-val--warn" : ""}`}
                            >
                              {p.vitals.spo2}
                              <span className="db-table-unit">%</span>
                            </span>
                          </td>
                          <td>
                            <span
                              className={`db-table-val ${parseFloat(p.vitals.temp) > 38 ? "db-table-val--warn" : ""}`}
                            >
                              {p.vitals.temp}
                              <span className="db-table-unit">°C</span>
                            </span>
                          </td>
                        </tr>
                      ))}
                ...
                <div className="db-notice">
                  Values in red indicate readings outside normal range. Verify
                  with patient immediately.
                </div>
              </>
            )}
              ...
              <>
                {visAlerts.length === 0 ? (
                  ...
                  visAlerts.map((a) => (
                    <div
                      key={a.id}
                      className={`db-alert-card ${alertBorder(a.type)}`}
                    >
                      <div className="db-alert-left">
                        <div
                          className={`db-alert-dot ${alertDot(a.type)}`}
                        ></div>
                        <div>
                          <div className="db-alert-patient">
                            {a.patient} ·{" "}
                      ...
                      >
                        <span
                          className={`db-badge db-badge--${a.type === "critical" ? "critical" : a.type === "caution" ? "caution" : "info"}`}
                        >
                          {a.type}
                        </span>
                        <button
            ...
            {active === "appointments" && (
              <>
                  ...
                  .map((a) => (
                    <div key={a.id} className="db-appt-card">
                      <div className="db-appt-time-box">
                        <div className="db-appt-time-hour">
                          {a.time.split(":")[0]}
                        </div>
                        <div className="db-appt-time-ampm">
                          {a.time.split(" ")[1]}
                        </div>
                      </div>
                ...
                <div className="db-sub-title" style={{ marginTop: "1.5rem" }}>
                  Completed
                </div>
                {appointments
              ...
              <>
                <div className="db-progress-card">
                  <div className="db-progress-label">
                    {givenMeds} of {meds.length} medications administered today
                  </div>
                  <div className="db-progress-bar">
                    <div
                      className="db-progress-fill"
                      style={{ width: `${(givenMeds / meds.length) * 100}%` }}
                    ></div>
                  </div>
                </div>
                {meds.map((m) => (
                  <div
                    key={m.id}
                    className={`db-med-card ${m.given ? "db-med-card--taken" : ""}`}
                  >
            ...
            {active === "tasks" && (
              <>
                <div className="db-task-input-row">
                  ...
                  />
                  <button className="db-add-btn" onClick={addTask}>
                    Add
                  </button>
                </div>

                <div className="db-sub-title">
                  To do — {tasks.filter((t) => !t.done).length} remaining
                </div>
                {tasks
                  .filter((t) => !t.done)
                  .map((t) => (
                    <div
                      key={t.id}
                      className="db-task-card"
                      onClick={() => toggleTask(t.id)}
                    >
                      <div
                        className={`db-task-circle db-task-circle--${t.priority}`}
                      ></div>
                      <div className="db-task-text">{t.text}</div>
                      <span
                        className={`db-badge ${t.priority === "high" ? "db-badge--critical" : t.priority === "medium" ? "db-badge--caution" : "db-badge--neutral"}`}
                      >
                        {t.priority}
                      </span>
                    </div>
                  ...
                  <>
                    ...
                    {tasks
                      ...
                      .map((t) => (
                        <div
                          key={t.id}
                          className="db-task-card db-task-card--done"
                          onClick={() => toggleTask(t.id)}
                        >
                          <div className="db-task-circle db-task-circle--done-check">
                            ✓
                          </div>
                          <div className="db-task-text db-task-text--done">
                            {t.text}
                          </div>
                        </div>
                      ))}
